...
def emailSpoofDetection(header, emailDomain):
    ...
    if ("pass" in dkim["result"] and "pass" in spf["result"] and "pass" in dmarc["result"] and emailDomain in dkim["domain"]):
        outcome = {'Congratulations!!! The Email address is valid'}
    else:
        outcome = {'Oh no, someone sent you a Spoofing message or you entered wrong details'}
    return outcome

cprint("Welcome to our Email Spoofing Detector", "green", attrs=["bold"])

header = 'paste-here-the-header'
